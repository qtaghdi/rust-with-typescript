...
Rust는 GC가 없습니다. 대신 **[Ownership](/glossary/#메모리--소유권)** 시스템으로 컴파일러가 어느 시점에 메모리를 해제할지를 빌드 타임에 결정하고, 그 해제 코드를 자동으로 삽입합니다.
...
- 비동기 작업 후 UI 상태 갱신은 "실패/성공"을 분기한다 → Rust의 [`Result<T, E>`](/glossary/#에러-처리) 모델과 유사하다.
